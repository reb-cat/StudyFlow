...
// Shared version that works in both client and server environments.
...
export type AssignmentLike = {
  ...
  dueDate?: Date | null; // Date object if present
};
...
export function normalizeAssignment(a: AssignmentLike, now = new Date()): NormalizedAssignment {
  const courseLabel = a.course ?? a.courseName ?? a.subject ?? null;

  // Handle existing dueDate (Date object) or dueAt (ISO string)
  let existingDueDate: string | null = null;
  if (a.dueAt) {
    existingDueDate = a.dueAt;
  } else if (a.dueDate) {
    existingDueDate = a.dueDate.toISOString();
  }

  // 1) Effective due date: prefer existing due date; otherwise infer from title or instructions
  const inferred =
    inferDueDateFromText(a.title, now) ||
    inferDueDateFromText(a.instructions || '', now) ||
    null;

  const effectiveDueAt = existingDueDate ?? (inferred ? inferred.toISOString() : null);
